...
#if !defined(MSDOS_SUPPORTED_ANTIQUE) && !defined(WIN32)
#include <signal.h>
#include <fcntl.h>
#include <time.h>
#include <unistd.h>
...
#ifdef WIN32
static char Win32Omegalib[ MAX_PATH ];
#endif
...
char *argv[];
{
  ...
  {
      //
      // Grab the path to omega.exe, and make Omegalib relative to that.
      //
      HMODULE hMod;
      int i;
      ...
      Omegalib = &Win32Omegalib[ 0 ];
      Omegalib[ 0 ] = '\0';
      if( GetModuleFileNameA( hMod, Omegalib, MAX_PATH ) )
      {
          i = strlen( Omegalib );
          while( Omegalib[ i ] != '\\' )
          {
